...
public class RealRepository implements Repository {
  ...
  private static Logger log = LoggerFactory.getLogger(RealRepository.class);
  ...
  private final Timer findTimer = new Timer();
  private final Slf4jReporter reporter;

  public RealRepository(Connection conn) throws Exception {
    ...
    reporter = Slf4jReporter.forRegistry(registry)
        .outputTo(LoggerFactory.getLogger(RealRepository.class.getName() + ".metrics"))
  ...
  public EquityTrade get(TradeId id, int version) throws Exception {
    ...
    log.debug("Get a single trade by row key={}", rowKey);
  ...
  public List<EquityTrade> find(Query query) throws Exception {
    ...
    List<EquityTrade> trades = withTimer(findTimer, "find", () -> doFind(scan));

    return trades;

  }

  private List<EquityTrade> doFind(Scan scan) throws IOException {
    ...
    try (ResultScanner scanner = table.getScanner(scan)) {
      ...
      while (rowIt.hasNext()) {
        ...
        result.add(trade);
        log.debug("  read {}, {}", trade.getIdVersion(), trade);
      }
      log.debug("Done. Read {} trades", result.size());
      return result;
    }
  }
  ...
  public void put(EquityTrade trade) throws Exception {
    ...
    log.debug("Saving with rowkey={}", rowKey);
