...
ExchangeService.prototype.generateTickerRow = function (symbol) {
    let price = this.random(50, 900);
    let open = price - this.random(5,19);
    let chg = this.random(5,19);
    let close = price - chg;
    return {
        symbol:symbol,
        price:price,
        ...
        bidq: this.random(100,9999),
        askq: this.random(500,9999),
        open: open,
        close: close,
        high: open + this.random(19,35),
        low: open - this.random(19,35),
        chg: chg
    }
};
...
ExchangeService.prototype.applyDeltasToTickerData = function () {
    ...
    let properties = ['price', 'bid', 'ask', 'bidq', 'askq', 'open', 'close', 'high', 'low', 'chg'];
    ...
    symbolsToAlter.forEach(function(symbol) {
        //alert(JSON.stringify(self.tickerData[symbol],null,4));
        self.tickerData[symbol] = {
            symbol:symbol,
            price: self.tickerData[symbol].price,
            bid: self.tickerData[symbol].bid,
            ask: self.tickerData[symbol].ask,
            bidq: self.tickerData[symbol].bidq,
            askq: self.tickerData[symbol].askq,
            open: self.tickerData[symbol].open,
            close: self.tickerData[symbol].close,
            high: self.tickerData[symbol].high,
            low: self.tickerData[symbol].low,
            chg: self.tickerData[symbol].chg
        };
...
ExchangeService.prototype.createTickerDetail = function (symbol) {
    ...
    let open = this.formatNumber(ticker.price); 
